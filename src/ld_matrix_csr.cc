...
int LdMatrixCsr::num_ld_r_snp(SnpIndex snp_index) {
  const int chr_label = mapping_.chrnumvec()[snp_index.index()];
  const LdMatrixCsrChunk& chunk = chunks_forward_[chr_label];
  return chunk.num_ld_r2(snp_index.index());
}

int LdMatrixCsr::num_ld_r_tag(TagIndex tag_index) {
  const int chr_label = mapping_.chrnumvec()[mapping_.tag_to_snp()[tag_index.index()]];
  const LdMatrixCsrChunk& chunk = chunks_reverse_[chr_label];
  return chunk.num_ld_r2(tag_index.index());
}
...
void LdSum::dump(BgmgSerializer* serializer) {
  serializer->dump_vector(&ld_sum_r2_below_r2min_);
  serializer->dump_vector(&ld_sum_r2_above_r2min_);
  serializer->dump_vector(&ld_sum_r4_above_r2min_);
  serializer->dump_vector(&ld_tag_sum_r2_below_r2min_);
  serializer->dump_vector(&ld_tag_sum_r2_below_r2min_);
  serializer->dump_vector(&ld_tag_sum_r4_above_r2min_);
}

void LdMatrixCsrChunk::dump(BgmgSerializer* serializer) {
  serializer->dump_vector(&coo_ld_);
  serializer->dump_vector(&csr_ld_key_index_);
  serializer->dump_vector(&csr_ld_val_index_offset_);
  serializer->dump_vector(&csr_ld_val_index_packed_);
  serializer->dump_vector(&csr_ld_r_);
  serializer->dump_value(&key_index_from_inclusive_);
  serializer->dump_value(&key_index_to_exclusive_);
  serializer->dump_value(&chr_label_);
}

void LdMatrixCsr::dump(BgmgSerializer* serializer) {
  if ((ld_sum_adjust_for_hvec_ == nullptr) || (ld_sum_ == nullptr))
    BGMG_THROW_EXCEPTION(::std::runtime_error("unable to serialize, call set_chrnumvec() first" ));

  for (int i = 0; i < chunks_forward_.size(); i++) chunks_forward_[i].dump(serializer);
  for (int i = 0; i < chunks_reverse_.size(); i++) chunks_reverse_[i].dump(serializer);
  ld_sum_adjust_for_hvec_->dump(serializer);
  ld_sum_->dump(serializer);
}