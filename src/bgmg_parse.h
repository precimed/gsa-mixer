...
class ChrBpRange {
public:
  ChrBpRange(int chrnum, int from_bp, int to_bp);
  bool contains(int chr, int bp) const;
private:
  int chrnum_;
  int from_bp_;
  int to_bp_;
...
public:
  ...
  SumstatFile() {}
  void read(const BimFile& bim, std::string filename, bool allow_ambiguous_snps);
