...
double BgmgCalculator::calc_unified_univariate_cost(int trait_index, int num_components, int num_snp, float* pi_vec, float* sig2_vec, float sig2_zeroA, float sig2_zeroC, float sig2_zeroL, float* aux) {
  ...
  if (aux_option_ == AuxOption_Gradients) {
    if (cost_calculator_ != CostCalculator_Sampling) BGMG_THROW_EXCEPTION(::std::runtime_error("AuxOption_Gradients is only supported for CostCalculator_Sampling"));
    if (sig2_zeroC != 1.0) BGMG_THROW_EXCEPTION(::std::runtime_error("AuxOption_Gradients does not support multiplicative inflation, please use sig2_zeroC=1.0)"));
  }
...
double BgmgCalculator::calc_unified_univariate_cost_sampling(int trait_index, int num_components, int num_snp, float* pi_vec, float* sig2_vec, float sig2_zeroA, float sig2_zeroC, float sig2_zeroL, float* aux, const float* weights) {
  ...
  std::stringstream ss;
  const int k_max = k_max_;
  ss << "calc_unified_univariate_cost_sampling(trait_index=" << trait_index << ", num_components=" << num_components << ", num_snp=" << num_snp << ", sig2_zeroA=" << sig2_zeroA << ", sig2_zeroC=" << sig2_zeroC << ", sig2_zeroL=" << sig2_zeroL << ", k_max=" << k_max << ")";
  LOG << ">" << ss.str();
  ...
  const double pi_k = 1.0 / static_cast<double>(k_max);
  double log_pdf_total = 0.0;
  int num_infinite = 0;

  const int gradients_size = ((aux != nullptr) && (aux_option_ == AuxOption_Gradients)) ? (num_components * num_snp_ + 2) : 0;
  std::valarray<float> gradients_global(0.0f, gradients_size);

#pragma omp parallel
  {
    LdMatrixRow ld_matrix_row;
    std::vector<float> tag_delta2(k_max, 0.0f);
    std::valarray<double> tag_kpdf(0.0f, k_max);
    std::vector<float> gradient_inc(k_max, 0.0f);
    std::valarray<float> gradients_local(0.0f, gradients_size);
...
#pragma omp for schedule(dynamic, kOmpDynamicChunk) reduction(+: log_pdf_total, num_infinite)
    for (int deftag_index = 0; deftag_index < num_deftag; deftag_index++) {
      ...
      MultinomialSampler subset_sampler((seed_ > 0) ? seed_ : (seed_ - 1), 1 + tag_to_snp_[tag_index], k_max, num_components);
      const float sig2_zero = sig2_zeroA + ld_tag_sum_r2_below_r2min_adjust_for_hvec[tag_index] * nvec[tag_index] * sig2_zeroL;
      find_unified_univariate_tag_delta_sampling(num_components, k_max, pi_vec, sig2_vec, sig2_zeroC, tag_index, &nvec[0], &hvec[0], &tag_delta2, &subset_sampler, &ld_matrix_row);

      double pdf_tag = 0.0;
      double average_tag_delta2 = 0.0;
      for (int k = 0; k < k_max; k++) {
        const float s2 = tag_delta2[k] + sig2_zero;
        ...
        tag_kpdf[k] = pdf;
        if (censoring) {
          // line below is confusing - the gradient of censored_cdf(z_max, s) is closely related to gaussian_pdf(z_max, s), times (z_max/s2). 
          // Look into supplementary details for mathematical formulas - they are easier to follow than code.
          gradient_inc[k] = weights[tag_index] * pi_k * gaussian_pdf<FLOAT_TYPE>(z_max, s) * z_max / s2;
        } else {
          gradient_inc[k] = weights[tag_index] * pi_k * pdf * (tag_z*tag_z - s2) / (2.0*s2*s2);
        }
      }

      for (int k = 0; k < k_max; k++) gradient_inc[k] /= pdf_tag;
      ...
      if ((aux != nullptr) && (aux_option_ == AuxOption_TagPdfErr)) aux[tag_index] = sqrt(((tag_kpdf - pdf_tag) * (tag_kpdf - pdf_tag)).sum() / (static_cast<double>(k_max) - 1.0));

      if ((aux != nullptr) && (aux_option_ == AuxOption_Gradients)) {
        // compute gradients w.r.t. sig2_zero and sig2_zeroL, and store them in the last two cells of gradients_local 
        float gradient_inc_sum = 0; for (int k_index = 0; k_index < k_max; k_index++) gradient_inc_sum += gradient_inc[k_index];
        gradients_local[gradients_local.size() - 2] += gradient_inc_sum;
        gradients_local[gradients_local.size() - 1] += gradient_inc_sum * ld_tag_sum_r2_below_r2min_adjust_for_hvec[tag_index] * nvec[tag_index];

        // compute gradients w.r.t all sig2_beta
        MultinomialSampler subset_sampler_gradients((seed_ > 0) ? seed_ : (seed_ - 1), 1 + tag_to_snp_[tag_index], k_max, num_components);
        find_unified_univariate_gradients_sampling(num_components, pi_vec, sig2_vec, sig2_zeroC, tag_index, &nvec[0], &hvec[0], &gradients_local, &subset_sampler_gradients, &ld_matrix_row, gradient_inc);
      }
...
#pragma omp critical
    {
      gradients_global += gradients_local;
    }
  }

  if ((aux != nullptr) && (aux_option_ == AuxOption_Gradients))
    for (int i = 0; i < gradients_size; i++)
      aux[i] = gradients_global[i];
...
double BgmgCalculator::calc_unified_univariate_cost_smplfast(int trait_index, int num_components, int num_snp, float* pi_vec, float* sig2_vec, float sig2_zeroA, float sig2_zeroC, float sig2_zeroL, float* aux, const float* weights) {
  ...
  std::stringstream ss;
  const int k_max = k_max;
  ss << "calc_unified_univariate_cost_smplfast(trait_index=" << trait_index << ", num_components=" << num_components << ", num_snp=" << num_snp << ", sig2_zeroA=" << sig2_zeroA << ", sig2_zeroC=" << sig2_zeroC << ", sig2_zeroL=" << sig2_zeroL << ", k_max=" << k_max << ")";
  LOG << ">" << ss.str();
...
#pragma omp for schedule(dynamic)
    for (int k_index = 0; k_index < k_max; k_index++) {
      MultinomialSampler subset_sampler((seed_ > 0) ? seed_ : (seed_ - 1), 1 + k_index, num_snp_, num_components);
      find_unified_univariate_tag_delta_smplfast(num_components, k_max, pi_vec, sig2_vec, sig2_zeroC, k_index, &nvec[0], &hvec[0], &tag_delta2, &subset_sampler, &ld_matrix_row);
      for (int deftag_index = 0; deftag_index < num_deftag; deftag_index++) {
        const int tag_index = deftag_indices[deftag_index];
        const float sig2_zero = sig2_zeroA + ld_tag_sum_r2_below_r2min_adjust_for_hvec[tag_index] * nvec[tag_index] * sig2_zeroL;
...
void BgmgCalculator::find_unified_univariate_tag_delta_sampling(int num_components, int k_max, float* pi_vec, float* sig2_vec, float sig2_zeroC, int tag_index, const float* nvec, const float* hvec, std::vector<float>* tag_delta2, MultinomialSampler* subset_sampler, LdMatrixRow* ld_matrix_row) {
  // NB! Remember that this function must be in sync with find_unified_univariate_GRADIENTS_sampling() - see below
  tag_delta2->assign(k_max, 0.0f);
  ld_matrix_csr_.extract_tag_row(TagIndex(tag_index), ld_matrix_row);
  auto iter_end = ld_matrix_row->end();
  ...
  for (auto iter = ld_matrix_row->begin(); iter < iter_end; iter++) {
    ...
    int sample_global_index = k_max - subset_sampler->sample_shuffle();
    ...
    assert(sample_global_index==k_max);
  }
...
void BgmgCalculator::find_unified_univariate_gradients_sampling(int num_components, float* pi_vec, float* sig2_vec, float sig2_zeroC, int tag_index, const float* nvec, const float* hvec, std::valarray<float>* gradients,  MultinomialSampler* subset_sampler, LdMatrixRow* ld_matrix_row, const std::vector<float>& gradient_inc) {
  // NB! Remember that this function must be in sync with find_unified_univariate_TAG_DELTA_sampling() - see above
  ld_matrix_csr_.extract_tag_row(TagIndex(tag_index), ld_matrix_row);
  auto iter_end = ld_matrix_row->end();

  float gradient_inc_sum = 0;
  const int k_max = k_max_;
  for (int k_index = 0; k_index < k_max; k_index++) gradient_inc_sum += gradient_inc[k_index];
  ...
  for (auto iter = ld_matrix_row->begin(); iter < iter_end; iter++) {
    ...
    for (int comp_index = 0; comp_index < num_components; comp_index++) {
      ...
      float gradient_inc_val = 0.0;
      float gradient_inc_sign = 1.0;
      if (pi_vec[index] > 0.5f) { // for pi_val close to 1.0 it'll be faster to compute total, and deduct selected (1-pi_val) samples at random
        gradient_inc_val = gradient_inc_sum;
        gradient_inc_sign = -1;
      }

      const int num_samples=subset_sampler->counts()[comp_index];
      for (int sample_index=0; sample_index < num_samples; sample_index++, sample_global_index++) {
        gradient_inc_val += gradient_inc_sign * gradient_inc[indices[sample_global_index]];
      }

      (*gradients)[index] += (gradient_inc_val * r2_hval_nval_sig2zeroC);
    }
    assert(sample_global_index==k_max);
  }
}

void BgmgCalculator::find_unified_univariate_tag_delta_smplfast(int num_components, int k_max, float* pi_vec, float* sig2_vec, float sig2_zeroC, int k_index, const float* nvec, const float* hvec, std::vector<float>* tag_delta2, MultinomialSampler* subset_sampler, LdMatrixRow* ld_matrix_row) {
...
void BgmgCalculator::find_unified_bivariate_tag_delta_sampling(int num_snp, int k_max, float* pi_vec, float* sig2_vec, float* rho_vec, float* sig2_zeroA, float* sig2_zeroC, float* sig2_zeroL, float rho_zeroA, float rho_zeroL, int tag_index, const float* nvec1, const float* nvec2, const float* hvec, std::vector<float>* tag_delta20, std::vector<float>* tag_delta02, std::vector<float>* tag_delta11, MultinomialSampler* subset_sampler, LdMatrixRow* ld_matrix_row) {
  tag_delta20->assign(k_max, 0.0f); tag_delta02->assign(k_max, 0.0f); tag_delta11->assign(k_max, 0.0f);
  ld_matrix_csr_.extract_tag_row(TagIndex(tag_index), ld_matrix_row);
  auto iter_end = ld_matrix_row->end();
...
void BgmgCalculator::find_unified_bivariate_tag_delta_smplfast(int num_snp, int k_max, float* pi_vec, float* sig2_vec, float* rho_vec, float* sig2_zeroA, float* sig2_zeroC, float* sig2_zeroL, float rho_zeroA, float rho_zeroL, int k_index, const float* nvec1, const float* nvec2, const float* hvec, std::vector<float>* tag_delta20, std::vector<float>* tag_delta02, std::vector<float>* tag_delta11, MultinomialSampler* subset_sampler, LdMatrixRow* ld_matrix_row) {
...
int64_t BgmgCalculator::calc_unified_univariate_pdf(int trait_index, int num_components, int num_snp, float* pi_vec, float* sig2_vec, float sig2_zeroA, float sig2_zeroC, float sig2_zeroL, int length, float* zvec, float* pdf) {
  ...
  std::stringstream ss;
  const int k_max = k_max_pdf_;
  ss << "calc_unified_univariate_pdf(trait_index=" << trait_index << ", num_components=" << num_components << ", num_snp=" << num_snp << ", sig2_zeroA=" << sig2_zeroA << ", sig2_zeroC=" << sig2_zeroC << ", sig2_zeroL=" << sig2_zeroL << ", length=" << length << ", k_max=" << k_max << ")";
  LOG << ">" << ss.str();
...
int64_t BgmgCalculator::calc_unified_univariate_power(int trait_index, int num_components, int num_snp, float* pi_vec, float* sig2_vec, float sig2_zeroA, float sig2_zeroC, float sig2_zeroL, float zthresh, int length, float* nvec, float* svec_num, float* svec_denom) {
  std::stringstream ss;
  const int k_max = k_max_pdf_;
  ss << "calc_unified_univariate_power(trait_index=" << trait_index << ", num_components=" << num_components << ", num_snp=" << num_snp << ", sig2_zeroA=" << sig2_zeroA << ", sig2_zeroC=" << sig2_zeroC << ", sig2_zeroL=" << sig2_zeroL << ", zthresh=" << zthresh << ", length=" << length << ", k_max=" << k_max << ")";
  LOG << ">" << ss.str();
...
#pragma omp for schedule(dynamic, kOmpDynamicChunk)
    for (int deftag_index = 0; deftag_index < num_deftag; deftag_index++) {
      ...
      MultinomialSampler subset_sampler((seed_ > 0) ? seed_ : (seed_ - 1), 1 + tag_to_snp_[tag_index], k_max, num_components);
      const double tag_weight = static_cast<double>(weights_[tag_index]);
      find_unified_univariate_tag_delta_sampling(num_components, k_max, pi_vec, sig2_vec, sig2_zeroC, tag_index, &nvec_dummy[0], &hvec[0], &tag_delta2, &subset_sampler, &ld_matrix_row);
...
#pragma omp critical
  ...
  for (int i = 0; i < length; i++) svec_num[i] = static_cast<float>(s_numerator_global[i]);
  for (int i = 0; i < length; i++) svec_denom[i] = static_cast<float>(s_denominator_global[i]);
  LOG << "<" << ss.str() << ", num_deftag=" << num_deftag << ", elapsed time " << timer.elapsed_ms() << "ms";
  return 0;
}
...
int64_t BgmgCalculator::calc_unified_univariate_delta_posterior(int trait_index, int num_components, int num_snp, float* pi_vec, float* sig2_vec, float sig2_zeroA, float sig2_zeroC, float sig2_zeroL, int length, float* c0, float* c1, float* c2) {
  std::stringstream ss;
  const int k_max = k_max_;
  ss << "calc_unified_univariate_delta_posterior(trait_index=" << trait_index << ", num_components=" << num_components << ", num_snp=" << num_snp << ", sig2_zeroA=" << sig2_zeroA << ", sig2_zeroC=" << sig2_zeroC << ", sig2_zeroL=" << sig2_zeroL << ", length=" << length << ", k_max=" << k_max << ")";
  LOG << ">" << ss.str();
...
double BgmgCalculator::calc_unified_bivariate_cost_sampling(int num_snp, float* pi_vec, float* sig2_vec, float* rho_vec, float* sig2_zeroA, float* sig2_zeroC, float* sig2_zeroL, float rho_zeroA, float rho_zeroL, float* aux, const float* weights) {
  ...
  std::stringstream ss;
  const int k_max = k_max_;  
  ss << "calc_unified_bivariate_cost_sampling(" << find_bivariate_params_description(num_snp, pi_vec, sig2_vec, rho_vec, sig2_zeroA, sig2_zeroC, sig2_zeroL, rho_zeroA, rho_zeroL) << ", k_max=" << k_max << ")";
  LOG << ">" << ss.str();
...
#pragma omp parallel
  {
    LdMatrixRow ld_matrix_row;
    std::vector<float> tag_delta20(k_max, 0.0f);
    std::vector<float> tag_delta02(k_max, 0.0f);
    std::vector<float> tag_delta11(k_max, 0.0f);
    std::valarray<double> tag_kpdf(0.0f, k_max);
...
#pragma omp for schedule(dynamic, kOmpDynamicChunk) reduction(+: log_pdf_total, num_infinite)
    for (int deftag_index = 0; deftag_index < num_deftag; deftag_index++) {
      ...
      find_unified_bivariate_tag_delta_sampling(num_snp, k_max, pi_vec, sig2_vec, rho_vec, sig2_zeroA, sig2_zeroC, sig2_zeroL, rho_zeroA, rho_zeroL, tag_index, &nvec1_[0], &nvec2_[0], &hvec[0], &tag_delta20, &tag_delta02, &tag_delta11, &subset_sampler, &ld_matrix_row);
...
double BgmgCalculator::calc_unified_bivariate_cost_smplfast(int num_snp, float* pi_vec, float* sig2_vec, float* rho_vec, float* sig2_zeroA, float* sig2_zeroC, float* sig2_zeroL, float rho_zeroA, float rho_zeroL, float* aux, const float* weights) {
  ...
  std::stringstream ss;
  const int k_max = k_max_;  
  ss << "calc_unified_bivariate_cost_smplfast(" << find_bivariate_params_description(num_snp, pi_vec, sig2_vec, rho_vec, sig2_zeroA, sig2_zeroC, sig2_zeroL, rho_zeroA, rho_zeroL) << ", k_max=" << k_max << ")";
  LOG << ">" << ss.str();
...
#pragma omp for schedule(dynamic, 1) 
    for (int k_index = 0; k_index < k_max; k_index++) {
      MultinomialSampler subset_sampler((seed_ > 0) ? seed_ : (seed_ - 1), 1 + k_index, num_snp_, num_components);
      find_unified_bivariate_tag_delta_smplfast(num_snp, k_max, pi_vec, sig2_vec, rho_vec, sig2_zeroA, sig2_zeroC, sig2_zeroL, rho_zeroA, rho_zeroL, k_index, &nvec1_[0], &nvec2_[0], &hvec[0], &tag_delta20, &tag_delta02, &tag_delta11, &subset_sampler, &ld_matrix_row);
      for (int deftag_index = 0; deftag_index < num_deftag; deftag_index++) {
        const int tag_index = deftag_indices[deftag_index];
...
int64_t BgmgCalculator::calc_unified_bivariate_pdf(int num_snp, float* pi_vec, float* sig2_vec, float* rho_vec, float* sig2_zeroA, float* sig2_zeroC, float* sig2_zeroL, float rho_zeroA, float rho_zeroL, int length, float* zvec1, float* zvec2, float* pdf) {
  std::stringstream ss;
  const int k_max = k_max_pdf_;
  ss << "calc_unified_bivariate_pdf(" << find_bivariate_params_description(num_snp, pi_vec, sig2_vec, rho_vec, sig2_zeroA, sig2_zeroC, sig2_zeroL, rho_zeroA, rho_zeroL) << ", k_max=" << k_max << ", length=" << length << ")";
  LOG << ">" << ss.str();
...
int64_t BgmgCalculator::calc_unified_bivariate_delta_posterior(int num_snp, float* pi_vec, float* sig2_vec, float* rho_vec, float* sig2_zeroA, float* sig2_zeroC, float* sig2_zeroL, float rho_zeroA, float rho_zeroL,
  ...
  std::stringstream ss;
  const int k_max = k_max_;  
  ss << "calc_unified_bivariate_delta_posterior(" << find_bivariate_params_description(num_snp, pi_vec, sig2_vec, rho_vec, sig2_zeroA, sig2_zeroC, sig2_zeroL, rho_zeroA, rho_zeroL) << ", k_max=" << k_max << ")";
  LOG << ">" << ss.str();
