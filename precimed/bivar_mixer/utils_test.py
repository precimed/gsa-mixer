...
def get_params():
    return UnivariateParams(pi=0.02, sig2_beta=0.0005, sig2_zero=1.5)

def get_true_params():
    return UnivariateParams(pi=0.1, sig2_beta=0.001, sig2_zero=1.2)
...
class LibBgmgMock(object):
    ...
    @property
    def totalhet(self):
        return 2 * np.sum(np.multiply(self._mafvec, 1-self._mafvec))

    def set_option(self, option, value):
        pass

    def calc_unified_univariate_cost(self, trait, pi_vec, sig2_vec, sig2_zeroA, sig2_zeroC=1, sig2_zeroL=0):
        if self.weights is None: return self.cost
        true_sig2_zeroA = get_true_params()._sig2_zero
        true_pi = get_true_params()._pi
        true_sig2beta = get_true_params()._sig2_beta
...
# py.test precimed/bivar_mixer/utils_test.py  -k test_univariate_fit_diffevo
def test_univariate_fit_diffevo():
    ...
    assert(isinstance(params, UnivariateParams))
    check_optimize_result(optimize_result_sequence[0][1], expected_cost_df=3)

# py.test precimed/bivar_mixer/utils_test.py  -k test_univariate_fit_neldermead
def test_univariate_fit_neldermead():
    libbgmg_mock = LibBgmgMock(num_snp=10, num_tag=5, make_weights=True)
    args = collections.namedtuple('Args', ['diffevo_fast_repeats', 'seed', 'load_params_file'])._make((3, 123, ''))
    params, optimize_result_sequence = apply_univariate_fit_sequence(args, libbgmg_mock, ['neldermead-fast'], init_params=get_params(), trait=1)
    print(params, optimize_result_sequence)
    assert(isinstance(params, UnivariateParams))
    check_optimize_result(optimize_result_sequence[0][1], expected_cost_df=3)

# py.test precimed/bivar_mixer/utils_test.py  -k test_univariate_fit_inflation
def test_univariate_fit_inflation():
    libbgmg_mock = LibBgmgMock(num_snp=10, num_tag=5, make_weights=True)
    args = collections.namedtuple('Args', ['diffevo_fast_repeats', 'seed', 'load_params_file'])._make((3, 123, ''))
    params, optimize_result_sequence = apply_univariate_fit_sequence(args, libbgmg_mock, ['inflation'], init_params=get_params(), trait=1)
    print(params, optimize_result_sequence)
    assert(isinstance(params, UnivariateParams))
    check_optimize_result(optimize_result_sequence[0][1], expected_cost_df=1)

# py.test precimed/bivar_mixer/utils_test.py  -k test_univariate_fit_infinitesimal
def test_univariate_fit_infinitesimal():
    libbgmg_mock = LibBgmgMock(num_snp=10, num_tag=5, make_weights=True)
    args = collections.namedtuple('Args', ['diffevo_fast_repeats', 'seed', 'load_params_file'])._make((3, 123, ''))
    params, optimize_result_sequence = apply_univariate_fit_sequence(args, libbgmg_mock, ['infinitesimal'], init_params=get_params(), trait=1)
    print(params, optimize_result_sequence)
    assert(isinstance(params, UnivariateParams))
    check_optimize_result(optimize_result_sequence[0][1], expected_cost_df=2)

# py.test precimed/bivar_mixer/utils_test.py  -k test_univariate_uncertainty
def test_univariate_uncertainty():
    libbgmg_mock = LibBgmgMock(num_snp=10, num_tag=5, make_weights=True)    
    num_ci_samples = 5
    params = get_true_params()
    ci, ci_sample = _calculate_univariate_uncertainty(
        UnivariateParametrization(params, libbgmg_mock, trait=1), 
        alpha=0.95, 
        totalhet=libbgmg_mock.totalhet,
        num_snps=libbgmg_mock.num_snp,
        num_samples=num_ci_samples)
    #print(ci, ci_sample)
    for k in ci.keys():
...
# py.test precimed/bivar_mixer/utils_test.py  -k test_bivariate_fit_sequence
def test_bivariate_fit_sequence():
    libbgmg_mock = LibBgmgMock(num_snp=10, num_tag=5, make_weights=True)
    args = collections.namedtuple('Args', ['diffevo_fast_repeats', 'seed', 'analysis',  'fit_sequence', 'load_params_file', 'trait1_params_file', 'trait2_params_file'])._make((3, 123, 'fit2', ['diffevo-fast', 'neldermead-fast', 'brute1', 'brent1'], None, get_params(), get_params()))
    params, _, _, optimize_result_sequence = apply_bivariate_fit_sequence(args, libbgmg_mock)
    #print(params, optimize_result_sequence)
    ...
    args = collections.namedtuple('Args', ['analysis', 'fit_sequence', 'load_params_file'])._make(('test2', ['load', 'inflation'], params))
    params, _, _, optimize_result_sequence = apply_bivariate_fit_sequence(args, libbgmg_mock)
    print(params, optimize_result_sequence)
    for optimize_result in optimize_result_sequence:
        check_optimize_result(optimize_result[1], expected_cost_df=None)
...
# py.test precimed/bivar_mixer/utils_test.py  -k test_parametrizations
...
def test_parametrizations(UnivariateParametrization, expected_vec):
    params = UnivariateParams(0.001, 1e-4, 1.23)
    pp=UnivariateParametrization(None, 1)
    vec=pp.params_to_vec(params)
    assert(np.all(np.isclose(vec, expected_vec)))
    params2 = pp.vec_to_params(vec)
    assert(np.all(np.isclose([params._pi, params._sig2_beta, params._sig2_zero],
                             [params2._pi, params2._sig2_beta, params2._sig2_zero])))
